...
void DisplayImage2() {
    ...
    glTranslatef(-0.2f, 0.0f, 0.0f); // Shift to the left side
    glRotatef(340, 0, 1, 0);
    glRotatef(20, 1, 0, 0);
    glRotatef(10, 1, 0, 0);

    // Calculate the final rotation angle to make the blue line perpendicular to the green line
    float finalRotationAngle = atan2(0.2f, 5.0f) * 180.0f / M_PI; // Angle between x-axis and y-axis

    // Rotate the entire scene slightly to the left around the y-axis
    glTranslatef(0.2f, 0.0f, 0.0f); // Move back to the original position
    glRotatef(-finalRotationAngle, 0, 1, 0); // Rotate to the left
...
void proiectieParalela(unsigned char c) {
  ...
  case 'w':
    ...
    break;
  case '1':
    glOrtho(-1, 6, -6, 1, -5, 20);
    break;
  case '2':
      glOrtho(-5, 15, -5, 15, -10, 20);
    break;
  case '3':
    glOrtho(-10, 10, -10, 10, -20, 20);
    break;
